...
class ImageDataset(Dataset):
    def __init__(self, prefix: str, transform=None):
        self.prefix = prefix
        self.filenames = [
            filename
            for filename in os.listdir(os.path.join("data", self.prefix + "_data"))
        ]
        self.labels = pd.read_csv(
            os.path.join("data", self.prefix + ".csv"), delimiter=","
        )
    ...
    def __getitem__(self, index: int):
        img_path = os.path.join("data", self.prefix + "_data", self.filenames[index])
        ...
        return (
            image,
            self.filenames_to_labels_map[
                self.prefix + "_data" + "/" + self.filenames[index]
            ],
        )
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(
        dataset, batch_size=args.batch_size, sampler=train_sampler, pin_memory=True
    )
    val_dataloader = DataLoader(
        dataset, batch_size=args.batch_size, sampler=valid_sampler, pin_memory=True
    )
